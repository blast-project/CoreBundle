    ...
    blast:
        configuration: #global configuration
            search: #list of admin services that will be available in the global search box (they will appear in the same order)
                add:
                    - blast_crm.admin.contact
                    - blast_crm.admin.organism
                    - blast_email.admin.email
            ...
            Sonata\AdminBundle\Datagrid\ListMapper:   # The class of objects that needs to be configured (here the "list" view)
                ...
                add:
                    ...
                    _list_actions: # add custom action to generic top bar actions in list view
                        ...
                        pouet:
                            route: admin_blast_crm_contact_list # can be a route or an action
                            params: []          # parameters to be used when calling the route
                            translation_domain: SilCRMBundle
                        glop:
                            route: admin_blast_crm_contact_list
...
The use of your blast.yml configuration files is done in a specific order, allowing inheritance, properties sharing, etc. It starts from a working CRUD generated by Sonata, extending the ```Blast\Bundle\CoreBundle\Admin\CoreAdmin``` and extended by a ```MyEntityAdminConcrete``` as explained in the next title. Then the ```libre-informatique/core-bundle``` works this way:

1. All the ```blast.yml``` files are loaded, across your bundles, in the order of their definition in your ```app/AppKernel.php```.
